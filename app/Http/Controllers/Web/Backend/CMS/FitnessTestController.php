...
{
    /**
     * show how it works page how it works section data and section item
     */
    public function index(Request $request)
    {
        if ($request->ajax()) {
            $itemData = CMS::where('page', 'how-it-works')->where('section', 'fitness-test')->where('name', 'card')->get();
            ...
            return DataTables::of($itemData)
                ...
                ->addColumn('image', function ($item) {
                    ...
                    return '<img src="' . $image . '" alt="Image" width="40" style="background-color: #7a2048"; padding: 8px>';
                })
        ...
        $data = CMS::where('page', 'how-it-works')->where('section', 'fitness-test')->where('name', 'item')->first();

        return view("backend.layouts.cms.fitness_test", compact("data"));
    }


    /**
     * update how-it-works header section
     **/
    ...
    public function storeItem(CmsRequest $request)
    {
        try {
            ...
            if ($request->hasFile('image')) {
                $imagePath = Helper::uploadImage($request->file('image'), 'cms/fitness-item');
    ...
    public function updateItem(CmsRequest $request, $id)
    {
        try {
            ...
            if ($request->hasFile('image')) {
                ...
                // Store new image
                $image_path = Helper::uploadImage($request->file('image'), 'cms/fitness-item');
